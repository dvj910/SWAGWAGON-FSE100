...
while 3
    ...
    if color == 5 %red
        ...
        brick.MoveMotorAngleRel(lm, 50, -900, 'Brake');
        pause(2);
    end
    if distance < 5
    ...
    if (distance > 45)
        ...
        brick.StopAllMotors('Brake');
        pause(1);
        brick.MoveMotorAngleRel(rm, 48, -200, 'Brake');
        brick.MoveMotorAngleRel(lm, 51, -200, 'Brake');
        pause(3);
        brick.beep();
        bumpIntoWall = false;
    end
    ...
    if color == 3 %green
        ...
        KeyBoardControls; %to stop the car from moving
    end
    if color == 2 %blue
        brick.StopAllMotors('Brake');
        for i = 1:2
